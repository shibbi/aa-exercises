...
def expensive_tastes
  ...
  execute(<<-SQL)
    ...
    JOIN (
      SELECT
        albums.price, COUNT(tracks.song) AS count
      FROM
        albums
      JOIN
      ...
      GROUP BY
        albums.price
      ) AS albums ON albums.asin = styles.album
    GROUP BY
      styles.style, albums.price
    ORDER BY
      (SUM(albums.price) / SUM(albums.count)) DESC
    LIMIT
      5;
  SQL
end
